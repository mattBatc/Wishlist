...
public class DatabaseInterface {
    ...
    public void delItem(@NonNull final AppDatabase db, Item item){
        db.itemDAO().deleteItem(item);
        Log.d(DatabaseInterface.TAG,"Deleted" + item.name);
    }

    public void addItem(@NonNull final AppDatabase db, Item item){
        db.itemDAO().insert(item);
        Log.d(DatabaseInterface.TAG,"Added" + item.name);
    }

    public void nukeList(@NonNull final AppDatabase db){
        db.itemDAO().deleteAll();
        Log.d(DatabaseInterface.TAG,"Nuked List");
    }

    public List<Item> getList(@NonNull final AppDatabase db){
